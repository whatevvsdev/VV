...
namespace MagicaVoxel
{
	...
	struct XYZIChunk
	{
		...
		static XYZIChunk from(u8*& from)
		{
			...
			Voxel* voxels_ptr = ((Voxel*)from) + 1; // Skip first (num_voxels)
			chunk.voxels.resize(chunk.num_voxels);
			memcpy(chunk.voxels.data(), voxels_ptr, chunk.num_voxels * sizeof(Voxel));

			from += sizeof(i32) + sizeof(Voxel) * chunk.voxels.size();
